...
const ComplementaryInsuranceForm = () => {
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      ...
      case 3:
        return formData.currentInsurer.trim() !== "";
      case 4:
  ...
  // Skip the "assureur actuel" step (3) when the user has no current insurance.
  const handleAdvance = useCallback(() => {
    if (currentStep === 2 && formData.hasCurrentInsurance === false) {
      goToStep(4);
    } else {
      nextStep();
    }
  ...
  const handlePrevious = useCallback(() => {
    if (currentStep === 4 && formData.hasCurrentInsurance === false) {
      goToStep(2);
    } else {
      previousStep();
    }
  ...
  return (
    <>
      <FormContainer
        ...
        product="complementaire"
        guideMessages={[
          t("forms.complementaryGuide.needsStep", { defaultValue: "Quels besoins veux-tu couvrir ? Coche tout ce qui compte." }),
          t("forms.complementaryGuide.hasInsuranceStep", { defaultValue: "As-tu déjà une complémentaire ? Ça évite les doublons inutiles." }),
          t("forms.complementaryGuide.insurerStep", { defaultValue: "Chez qui es-tu assuré aujourd'hui ?" }),
          t("forms.complementaryGuide.healthStep", { defaultValue: "Quelques questions de santé, en toute confidentialité." }),
          t("forms.complementaryGuide.familyStep", { defaultValue: "Ta situation familiale m'aide à cibler les bonnes garanties." }),
          t("forms.complementaryGuide.birthStep", { defaultValue: "Ta date de naissance, pour un tarif juste." }),
          t("forms.complementaryGuide.locationStep", { defaultValue: "Où habites-tu ? Les offres varient d'un canton à l'autre." }),
          undefined,
          undefined,
        ]}
        navigation={
      ...
      >
                ...
                {NEEDS_OPTIONS.map((opt) => {
                  ...
                  return (
                    <label
                      key={opt.key}
                      className={cn(
                        "flex items-center gap-2 sm:gap-3 px-2.5 py-2 sm:p-4 border rounded-lg cursor-pointer transition-all min-h-[44px]",
                        checked
                    ...
                    >
                      ...
                      <span className="text-xs sm:text-base flex-1 leading-snug">{opt.label}</span>
                    </label>
                  );
                })}
              </div>
        ...
        {/* Step 2: Assurance actuelle (oui / non) */}
          ...
          <div className="space-y-4">
            <FormFieldWrapper label="Situation familiale" required>
              <IllustratedChoice
                ...
                onValueChange={(value) => {
                  updateFormData({ familySituation: value });
                  notify();
                }}
                columns={4}
                options={[
        ...
        <FormStep isActive={currentStep === 8}>
          <div className="space-y-5 sm:space-y-6">
            <div className="hidden text-center mb-4 sm:mb-6">
